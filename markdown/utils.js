 ...
 */
exports.getAttrs = function(str, options) {
  ...
  const matches = new RegExp(
    exports.escapeRegExp(options.leftDelimiter) +
      "(.*)" +
      exports.escapeRegExp(options.rightDelimiter)
  ).exec(str);

  if (matches) {
    attrs.push(["v-if", matches[1]]);
  }
...
exports.hasDelimiters = function(where, options) {
  if (!where) {
    throw new Error(
      'Parameter `where` not passed. Should be "start", "middle", "end" or "only".'
    );
  }
  ...
  return function(str) {
    ...
    let rightDelimiterMinimumShift =
